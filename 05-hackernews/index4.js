...
http.createServer(function (req, res) {
    ...
    //通过你url模块，调用url.parse()返回的是一个对象，该方法解析用户请求的url（req.url）
    ...
    if (req.url === '/' || req.url === '/index' && req.method === 'get') {
        ...
        //此处读取文件的时候可以直接写一个utf-8,回调函数中的data就是一个字符串了
        fs.readFile(path.join(__dirname,"data","data.json"),'utf-8',function (err,data) {
            if(err && err.code!=='ENOEND'){
                throw err;
            }
            var list=JSON.parse(data||'[]');
            list.push(urlObj.query);
            //2.把用户提交的新闻数据保存到data.json文件中
            //2.1把list数组中的数据写入到data.json文件中,SON.stringify()将json格式的数组转化为字符串
